"""EXP-009: Scheduled Sampling with prefix-confidence-gated loss.

Trains generator from scratch with scheduled sampling:
  - Epochs 1-5: pure teacher forcing (warmup).
  - Epochs 6-10: ss_prob ramps from 0 → 0.3.
  - Epochs 11-50: ss_prob = 0.3, modified loss active.

Compares against baseline_retrain (same data, same epochs, no SS).
"""
...
def main() -> None:
    ...
    run_dir = base_dir / "experiments/genrec_v2_exposure_bias/scheduled_sampling"
    run_dir.mkdir(parents=True, exist_ok=True)
    config.output_dir = str(run_dir)

    print(f"\n{'='*60}")
    print("SCHEDULED SAMPLING + MODIFIED LOSS")
    print(f"  ss_warmup={config.ss_warmup_epochs}, ss_max={config.ss_max_prob}")
    print(f"  ce_floor={config.ss_ce_floor}, epochs={config.epochs}, patience={config.patience}")
