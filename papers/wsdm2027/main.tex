...
where $\lambda$ is chosen by an identical \emph{held-out} validation procedure that we show is
essential to avoid over-trusting an overfit scorer. Across four settings spanning two base
...
decoder was never trained to calibrate.
...
of validation users that the scorer never trained on. We find this held-out selection is not a detail
but a requirement: selecting $\lambda$ on the scorer's own training users over-trusts an overfit
scorer and \emph{reverses} the gain on high-Scoring-Efficiency settings (Toys: $-13\%\to+3.2\%$ once
$\lambda$ is chosen honestly).
...
with a \textbf{leakage-free held-out $\lambda$-selection} procedure, and show the procedure---not just
the architecture---is the control variable that makes the method transfer (\S\ref{sec:method}).
...
5-seed listwise uses one shared $\lambda$. Selecting $\lambda$ on data the scorer trained on
over-trusts an overfit scorer and inflates $\lambda$; on high-Scoring-Efficiency Toys this
\emph{reversed} the test gain from $+3.2\%$ to $-13\%$. The transfer claim rests on holding this
...
leakage-free procedure yields a positive, Recall@10-significant listwise lift in all four settings
...
$\lambda^\star{=}0.1$ at SE 9\%, Amazon $\lambda^\star{=}0.02$ at SE 12--19\%). (iii) \emph{Held-out
vs.\ leaky $\lambda$}: leaky selection reverses Toys from $+3.2\%$ to $-13\%$.
...
small post-hoc scorer with a leakage-free held-out $\lambda$-selection closes it across diverse base
